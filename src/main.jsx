...
const USER_SHOP_FEE_RATE = 0.05;
const ledgerSaleMethod = (row) => row?.sale_method === 'shop' ? 'shop' : 'direct';
const ledgerNetSale = (value, method) => {
  const amount = ledgerNumber(value);
  if (amount <= 0) return 0;
  return ledgerSaleMethod({sale_method:method}) === 'shop' ? amount * (1 - USER_SHOP_FEE_RATE) : amount;
};
const newLedgerRow = () => ({
...
const rowFromTrade = (row) => ({
  ...
  sale_method: ledgerSaleMethod(row)
});
...
function SeafoodInputTable({form,setForm,used,remain}){
  ...
  return <div className="seafood-table-wrap compact-v24"><table className="seafood-table compact-v24"><thead><tr><th>어패류</th><th>등급</th><th>보유 수량</th><th>사용</th><th>남음</th></tr></thead><tbody>{SEAFOOD_ORDER.flatMap(type=>TIER_ORDER.map(t=>{ const key=sfKey(type,t); const row=form[key]||{}; const pieces=Number(row.pieces)||0; const sets=Number(row.sets)||0; const total=pieces+sets*SET_SIZE; return <tr key={key}><td className="seafood-name-cell"><span>{SEAFOOD_KO[type]}</span></td><td>{t}성</td><td className="qty-group-cell"><div className="qty-group"><label><span>낱개</span><input type="number" inputMode="numeric" min="0" value={row.pieces||''} onChange={e=>setCell(key,'pieces',e.target.value)} placeholder="0"/></label><label><span>64세트</span><input type="number" inputMode="numeric" min="0" value={row.sets||''} onChange={e=>setCell(key,'sets',e.target.value)} placeholder="0"/></label><strong>총 {formatOceanQty(total)}</strong></div></td><td className="number-cell">{formatOceanQty(used?.[key]||0)}</td><td className="number-cell">{formatOceanQty(remain?.[key]||0)}</td></tr>; }))}</tbody></table></div>;
}
...
function CraftResultCard({row,rank}){
  return <div className="craft-result-card"><div className="craft-result-head"><span className="rank-dot">{rank}</span><div><b>{row.name}</b><small>현재가 {oceanMoney(row.currentPrice)} · 최고가 {oceanMoney(row.max)} · {row.percent}%</small></div><strong>{oceanMoney(row.total)}</strong></div><div className="craft-price-line"><span>기본 제작 판매가 {oceanMoney(row.basePrice)}</span><span>{row.expertPrice===row.basePrice?'전문가 적용 동일':`전문가 적용 ${oceanMoney(row.expertPrice)}`}</span><span>제작 가능 {row.qty.toLocaleString()}개</span></div><div className="craft-materials">{row.materials.map(m=><span key={m.key} className={!m.tracked?'untracked':''}>{getMaterialDisplayName(m.key)} <b>x{oceanNum(m.qty)}</b></span>)}</div>{row.lack.length ? <em className="craft-lack">부족: {row.lack.map(x=>`${x.name} ${oceanNum(x.lack)}`).join(', ')}</em> : null}<details className="detail-box compact-detail"><summary>상세 보기</summary><div className="data-missing"><p>가격이 연결되지 않은 재료와 일부 획득처는 계산에 임의로 넣지 않습니다. 추적 불가 재료: {row.materials.filter(m=>!m.tracked).map(m=>getMaterialDisplayName(m.key)).join(', ') || '없음'}</p></div></details></div>;
}
...
function RecipeCard({row}){
  ...
  return <article className="recipe-card">
    <div className="recipe-main"><div className="recipe-title-line"><b>{row.name}</b><span>{row.kind || getRecipeKind(row)}</span></div>
      ...
      <div className="recipe-mats">{mats.length ? mats.map(([k,v])=><span key={k}>{getMaterialName(k)} <b>x{oceanNum(v)}</b></span>) : <span>재료 데이터 필요</span>}</div>
...
function MerchantLedgerPage({user,profile}){
  ...
  return <section className="merchant-page">
    ...
    <div className="ledger-summary-grid">
      ...
      <article><span>판매 합계</span><b>{ledgerMoney(totalSell)}</b><p>{soldRows.length.toLocaleString('ko-KR')}건 판매 완료 · 수수료 반영</p></article>
    ...
    <article className="ledger-sheet-card">
      ...
      <div className="ledger-table-wrap">
        <table className="ledger-table">
          <thead>
            <tr><th>#</th><th>날짜</th><th>아이템</th><th>매입가</th><th>예상 판매가</th><th>판매 방식</th><th>예상 수익</th><th>실제 판매가</th><th>상태</th><th>확정 순수익</th><th></th></tr>
          </thead>
          <tbody>
      ...
      <p className="ledger-hint">유저상점은 판매가에서 5% 수수료를 뺀 금액으로 예상 수익과 확정 순수익을 계산합니다.</p>
    </article>
  </section>;
}
